    ...
    if (
      ...
      switch (response.status) {
        case 204:
          ...
          break;
        case 401:
          logs(item.id, `❌ [JELLYFIN] Unauthorized (401). Check your API key or permissions.`);
          break;
        case 403:
          logs(item.id, `❌ [JELLYFIN] Forbidden (403). The server rejected the request.`);
          break;
        case 503:
          logs(item.id, `❌ [JELLYFIN] Service unavailable (503). The server is starting or temporarily unavailable.`);
          break;
        default:
